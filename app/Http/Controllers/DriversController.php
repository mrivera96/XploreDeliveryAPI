...
class DriversController extends Controller
{
    ...
    {
        try {
            $drivers = User::with(['agency', 'agency.city'])->where('isActivo', 1)
                ->where('idPerfil', 7)
    ...
    public function createDriver(Request $request)
    {
        ...
        try {
            ...
            if (UsersController::existeUsuario($rDriver['nickUsuario']) == 0) {
                $nDriver = new User();
    ...
    public function assignCategory(Request $request)
    {
        ...
        try {
            $driver = $request->idConductor;
            $category = $request->idCategoria;
            $exists = DriverCategories::where(['idConductor' => $driver, 'idCategoria' => $category])
                ->count();
            if ($exists == 0) {
                $nDC = new DriverCategories();
                $nDC->idConductor = $driver;
                $nDC->idCategoria = $category;
                $nDC->fechaRegistro = Carbon::now();
                $nDC->save();
                ...
                return response()->json(
                    [
                        'error' => 1,
                        'message' => 'La categoría ya se encuentra asignada al conductor.'
                    ],
                    500
                );
            }
            ...
            return response()->json(
                [
                    'error' => 1,
                    'message' => 'Ocurrió un error al asignar la categoría'
                ],
                500
            );
        }
    ...
    public function removeCategory(Request $request)
    {
        ...
        try {
            ...
            $exists = DriverCategories::where(['idConductor' => $driver, 'idCategoria' => $category])
                ->count();
            if ($exists > 0) {
                DriverCategories::where(['idConductor' => $driver, 'idCategoria' => $category])
                    ->delete();

                return response()->json(
                    [
                        'error' => 0,
                        'message' => 'Categoría removida correctamente.'
                    ],
                    500
                );
            }
            ...
            return response()->json(
                [
                    'error' => 1,
                    'message' => 'Ocurrió un error al remover la categoría'
                ],
                500
            );
        }
    ...
    public function getDriverCategories(Request $request)
    {
        $request->validate([
            'idConductor' => "required",
        ]);
        try {
            $driverCategories = DriverCategories::with(['driver', 'category'])
                ->where(['idConductor' => $request->idConductor])
                ->get();

            return response()->json(
                [
                    'error' => 0,
                    'data' => $driverCategories
                ],
                500
            );
        } catch (Exception $ex) {
