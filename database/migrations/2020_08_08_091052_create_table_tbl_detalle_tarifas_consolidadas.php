<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateTableTblDetalleTarifasConsolidadas extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('tblDetalleTarifaConsolidada', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('idTarifaDelivery')->unsigned();
            $table->double('radioMaximo');
            $table->string('dirRecogida');
        });

        Schema::table('tblDetalleTarifaConsolidada', function (Blueprint $table) {
            $table->foreign('idTarifaDelivery')
                ->references('idTarifaDelivery')
                ->on('clsTarifasDelivery');
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('tblDetalleTarifaConsolidada');
    }
}
